...
the robot to three joint configurations, executes the impedance controller
for 3\,s at each joint configuration, and logs the joint torques $\bm{\tau}=J^\top(q)F$
to separate CSV files for comparison.
The key section showing the three configurations and the experiment loop:
...
run_and_log(robot, model, "q1", "exp4_q1.csv", E, T_run);
move_to_config(robot, q2);
run_and_log(robot, model, "q2", "exp4_q2.csv", E, T_run);
move_to_config(robot, q3);
run_and_log(robot, model, "q3", "exp4_q3.csv", E, T_run);
