...
Experiment~4 demonstrates that the same Cartesian forces and moments
$\mathbf{F}=[-1,0,0,0,0,0]^\top\,\mathrm{N}$
produces different joint torques at different arm configurations,
because $\bm{\tau}=\mathbf{J}^\top(\mathbf{q})\mathbf{F}$ depends on the
Jacobian at the current joint configuration.
...
\begin{tabular}{@{}ll@{}}
\toprule
Config & Joint angles $\mathbf{q}$ [rad] \\
\midrule
$\mathbf{q}^{(1)}$ (home) &
  $[0,\ -\pi/4,\ 0,\ -3\pi/4,\ 0,\ \pi/2,\ \pi/4]$ \\
$\mathbf{q}^{(2)}$ (elbow up) &
  $[0,\ -\pi/6,\ 0,\ -2\pi/3,\ 0,\ \pi/2,\ \pi/4]$ \\
$\mathbf{q}^{(3)}$ (joint 1 rotated) &
...
for 3\,s at each joint configuration, and logs the joint torques $\bm{\tau}=\mathbf{J}^\top(\mathbf{q})\mathbf{F}$
