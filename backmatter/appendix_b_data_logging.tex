...
\begin{tikzpicture}[
  ...
  \node[box, fill=red!10]    (pc)    at (0,-5.4) {Analysis Machine\\(any OS, no robot)};
...
A typical file after a 5\,s run contains $5{,}000$ rows and is
...
\texttt{e\_px/y/z} & float & m   & Translational error $\mathbf{e}_p$ \\
\texttt{p\_ex/y/z} & float & m   & End-effector position $\mathbf{p}_e$ \\
...
\texttt{phi\_tilt\_deg}& float & deg & Tilt angle (free DOF) \\
\texttt{phi\_spin\_deg}& float & deg & Spin angle (stiff DOF) \\
\bottomrule
\end{tabular}
\end{center}
...
Run any script as:
\begin{lstlisting}[style=cppstyle, language=bash,
  caption={Run a plot script on measured data},
  label={lst:run_plot}]
...
%% =============================================================
%%  APPENDIX C (was B) – EXPERIMENT 1: CARTESIAN POSE TRACKING
%% =============================================================
