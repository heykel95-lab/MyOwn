...
Here \(R_{\mathrm{local}}\) rotates the local stiffness frame into the base
frame. It plays the same role as \(R_{\mathrm{task}}\) in
\cref{eq:full_task_frame_gain_transform}, applied to a stiffness frame that is
not tied to a surface. In the listing below the corresponding variable is named
\texttt{W\_local\_to\_base}.
...
\midrule
Desired point & \(p_d\) & \([0.50,0.00,0.40]^T\) & m \\
Initial error & \(e_p(0)\) & \([0.01,0,0]^T\) & m \\
Roll angle & \(\alpha\) & 20 & deg \\
Pitch angle & \(\beta\) & 30 & deg \\
Yaw angle & \(\gamma\) & 40 & deg \\
Local stiffness & \(K_{p,\mathrm{local}}\) & \(\operatorname{diag}(1000,300,100)\) & N/m \\
Virtual mass & \(M_p\) & \(I_3\) & kg \\
...
\(K_{p,\mathrm{base}}\) defines the shape and orientation of the
