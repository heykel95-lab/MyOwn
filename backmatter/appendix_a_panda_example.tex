...
\subsection*{Pose Error via $\mathbf{T}_d^{-1}\mathbf{T}_{EE}$}
...
The current pose at $\vq_0$ is $\mathbf{T}_{EE}$ with $\mathbf{R}_{EE}$ (from \abbr{FK})
and $\mathbf{p}_{EE}=[0.116,\;0.545,\;0.395]^\top\,\mathrm{m}$.

Following \cref{eq:T_err_expanded}, the pose error matrix is:

\begin{equation}
\mathbf{T}_{\mathrm{err}} = \mathbf{T}_d^{-1}\mathbf{T}_{EE} =
\begin{bmatrix}
...
\mathbf{e}_p = \mathbf{R}_d^\top(\mathbf{p}_{EE} - \mathbf{p}_d) =
\begin{bmatrix}
0.116 - 0.136 \\ 0 \\ 0
\end{bmatrix}
=
...
$\Delta\mathbf{R} = \mathbf{R}_d^\top\mathbf{R}_{EE} = \mathbf{R}_{EE}$.
Since the robot is at the desired orientation ($\mathbf{R}_{EE}\approx\mathbf{R}_d$),
...
($\dot{\mathbf{p}}_{EE}=\mathbf{0}$, $\boldsymbol{\omega}_{EE}=\mathbf{0}$),
the translational force and rotational moment from \cref{eq:imp_trans,eq:imp_rot} are:

\begin{equation}
\label{eq:F_panda}
\mathbf{f} = \mathbf{K}_p\,\mathbf{e}_p - \mathbf{D}_p\,\dot{\mathbf{p}}_{EE}
=
\begin{bmatrix}
50   & 0  & 0    \\
0    & 50 & 0    \\
...
2  & EE position           & $\mathbf{p}_{EE}$ & $[0.116,\;0.545,\;0.395]^\top\,\mathrm{m}$ \\
