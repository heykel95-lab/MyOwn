...
\(\mathbf p_{i-1}\) on its axis and its rotation axis
\(\mathbf z_{i-1}\), expressed in the base frame. They are extracted from
\(\mT_0^{i-1}(\vq_0)\) for \(i=1,\ldots,7\), including
\(\mT_0^0=I\) for joint~1, and are used to construct the Jacobian columns.

\begin{table}[htb]
\centering
\caption{Axis points \(\mathbf p_{i-1}\) and directions
         \(\mathbf z_{i-1}\) for joints \(i=1,\ldots,7\) at
...
\begin{bmatrix}
\mathbf{z}_{i-1} \times
(\mathbf{p}_{\mathrm{EE}} - \mathbf{p}_{i-1}) \\[3pt]
\mathbf{z}_{i-1}
\end{bmatrix}
\end{equation}
