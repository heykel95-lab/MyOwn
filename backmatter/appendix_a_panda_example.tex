...
(Cartesian pose tracking) with simulation results.
...
one 1\,ms cycle — position vector, Jacobian, pose error, impedance force,
and joint torques.
...
The Denavit--Hartenberg (\abbr{DH}) parameters published by Franka Emika \cite{FCI2023}
...
Applying the DH transformation chain of \cref{eq:fk} with $\vq = \vq_0$:
...
from \cref{eq:pos_vec}) is extracted from the upper-right column of
$\mT_0^7$:
...
Applying \cref{eq:jacobian_col} for each joint using the values from
\cref{tab:panda_fk} and the end-effector position \cref{eq:pe_panda}:

\begin{equation}
\mJ_k =
\begin{bmatrix}
\mathbf{z}_k \times (\mathbf{p}_e - \mathbf{p}_k) \\[3pt]
\mathbf{z}_k
\end{bmatrix}
\end{equation}
...
As explained in \cref{subsec:pinv}, the pseudo-inverse $\mJ^+$ is
\emph{not} required for the primary torque computation.
It is computed here for the null-space controller only.

\subsection{Computing $\mathbf{J}^+$}
...
As noted in \cref{subsec:pinv}, this thesis does not implement a null-space
controller.
...
For the Panda, the controlled variable is the full end-effector pose
$\mathbf{T}_e\in SE(3)$ with translational error
$\mathbf{e}_p\in\mathbb{R}^3$ and rotational error
$\mathbf{e}_R\in\mathbb{R}^3$ (see \cref{subsec:pose_error}).
The impedance matrices are $3\times3$ SPD matrices as defined in
\cref{subsec:KD_matrices}.

\subsection{Virtual Inertia Matrix $\mathbf{M}$}
...
\begin{itemize}
  ...
  \item Setting $\mM = \mI_6$ decouples all axes and simplifies the
        critical-damping formula to $D_i = 2\sqrt{K_i}$ for each axis $i$.
        The real link inertia and Coriolis effects are handled separately by
        $\bm{\tau}_g$ and $\bm{\tau}_c$ (see \cref{eq:full_with_comp}).
\end{itemize}

\subsection{Stiffness Matrices $\mathbf{K}_p$ and $\mathbf{K}_R$}

Following the virtual wall design of \cref{subsec:virtual_wall}, the
stiffness values in the wall frame are chosen as:
...
off-diagonal entries, as derived in \cref{eq:K_base}.

\subsection{Damping Matrices $\mathbf{D}_p$ and $\mathbf{D}_R$}

Critical damping with $\mathbf{M}_p=\mathbf{M}_R=\mathbf{I}_3$:
...
\subsection*{Pose Error via $\mathbf{T}_d^{-1}\mathbf{T}_e$}

Suppose the desired pose is $\mathbf{T}_d$ with $\mathbf{R}_d=\mathbf{I}_3$
and $\mathbf{p}_d=[0.136,\;0.545,\;0.395]^\top\,\mathrm{m}$
(the home position shifted $20\,\mathrm{mm}$ in $x$).
The current pose at $\vq_0$ is $\mathbf{T}_e$ with $\mathbf{R}_e$ (from \abbr{FK})
and $\mathbf{p}_e=[0.116,\;0.545,\;0.395]^\top\,\mathrm{m}$.

Following \cref{eq:T_err_expanded}, the pose error matrix is:

\begin{equation}
\mathbf{T}_{\mathrm{err}} = \mathbf{T}_d^{-1}\mathbf{T}_e =
\begin{bmatrix}
...
=
\begin{bmatrix}
\mathbf{R}_e & [-0.020,\;0,\;0]^\top \\
\mathbf{0}^\top & 1
\end{bmatrix}
\end{equation}

since $\mathbf{R}_d=\mathbf{I}_3$ so $\mathbf{R}_d^\top=\mathbf{I}_3$.

Reading off the sub-errors from the two blocks:

\paragraph{Translational error (upper-right column):}
\begin{equation}
\label{eq:panda_error}
\mathbf{e}_p = \mathbf{R}_d^\top(\mathbf{p}_e - \mathbf{p}_d) =
\begin{bmatrix}
0.116 - 0.136 \\ 0 \\ 0
\end{bmatrix}
=
...
$\Delta\mathbf{R} = \mathbf{R}_d^\top\mathbf{R}_e = \mathbf{R}_e$.
Since the robot is at the desired orientation ($\mathbf{R}_e\approx\mathbf{R}_d$),
$\Delta\mathbf{R}\approx\mathbf{I}$, giving
$\phi = \arccos\!\bigl(\tfrac{\mathrm{tr}(\mathbf{I})-1}{2}\bigr) = 0$
and $\mathbf{e}_R = \mathbf{0}$.

\subsection*{Impedance Wrench}

Assuming the robot is momentarily at rest
($\dot{\mathbf{p}}_e=\mathbf{0}$, $\boldsymbol{\omega}_e=\mathbf{0}$),
the translational force and rotational moment from \cref{eq:imp_trans,eq:imp_rot} are:

\begin{equation}
\label{eq:F_panda}
\mathbf{f} = \mathbf{K}_p\,\mathbf{e}_p - \mathbf{D}_p\,\dot{\mathbf{p}}_e
=
\begin{bmatrix}
50   & 0  & 0    \\
0    & 50 & 0    \\
...
Applying the control law \cref{eq:control_law} with $\mJ^\top \in \R{7\times6}$
...
\begin{itemize}
  \item $\tau_1 = \tau_2 = +0.545\,\mathrm{Nm}$:
        ...
        The positive sign means both joints rotate to pull the
        end-effector in the $+x$ direction (restoring towards $\mathbf{p}_d$).
  ...
  \item $\tau_4 = +0.322\,\mathrm{Nm}$:
        the elbow joint contributes positively; it has the largest
...
\begin{tabular}{@{}cp{3.2cm}p{1.6cm}p{6.4cm}@{}}
\toprule
Step & Quantity & Symbol & Value \\
\midrule
1  & Joint configuration   & $\vq$          & $[0, -45, 0, -135, 0, 90, 45]^\top$ deg \\
2  & EE position           & $\mathbf{p}_e$ & $[0.116,\;0.545,\;0.395]^\top\,\mathrm{m}$ \\
3  & Jacobian ($6\times7$) & $\mathbf{J}$   & See \cref{eq:J_panda} \\
4  & $\mathbf{T}_{\mathrm{err}}$ blocks & $\mathbf{e}_p,\mathbf{e}_R$ & $[-0.020,0,0]^\top$ m, $\mathbf{0}$ \\
...
// -- STEP 2: End-effector pose from libfranka ----------------------
Eigen::Affine3d T(Eigen::Matrix4d::Map(state.O_T_EE.data()));
Eigen::Vector3d p_e   = T.translation();    // [3x1] position
Eigen::Matrix3d R_e   = T.rotation();       // [3x3] orientation
...
// (pose error is now computed in STEP 5 via T_err; see below)

// -- STEP 5: Pose error via T_err = T_d^{-1} * T_e ---------------
...
// Rotational error: Delta_R = R_d^T * R_e
Eigen::Matrix3d Delta_R = R_d.transpose() * R_e;
...
// Translational force and rotational moment
...
//  (needed ONLY for null-space controller, NOT for primary torques)
// Pseudo-inverse and null-space projector (shown for completeness only).
// This thesis does NOT implement null-space control.
// The primary control law needs only J.transpose(), not J+.
Eigen::Matrix<double,7,6> Jp =
    J.transpose() * (J * J.transpose()).inverse();    // J+ [7x6]
// N = I7 - Jp*J  is the null-space projector; omitted here.
...
// -- FINAL COMMAND ------------------------------------------------
// Final command: primary impedance + gravity + Coriolis only
Eigen::Matrix<double,7,1> tau_cmd = tau_task + tau_g + tau_c;
//  (tau_null omitted: null-space control not implemented in this thesis)
