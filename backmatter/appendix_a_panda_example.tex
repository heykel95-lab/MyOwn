...
\cref{sec:nullspace_optimization_implementation}. In this numerical example,
the projector $\mathbf{N}$ is computed only to illustrate the structure of the
redundancy.
...
\cref{eq:surface_task_frame_matrix}, \(R_{\mathrm{surface}}=[\,n_s\;t_1\;t_2\,]\).
After transforming the normal-first local gains to the base frame, the
...
($\dot{p}_d=\mathbf{0}$, $\dot{\mathbf{p}}_{\mathrm{EE}}=\mathbf{0}$, $\boldsymbol{\omega}_{\mathrm{EE}}=\mathbf{0}$),
the positional force and rotational moment from
\cref{eq:positional_impedance_force,eq:rotational_impedance_moment} are:

\begin{equation}
\label{eq:F_panda}
\mathbf{f} = \mathbf{K}_p\,\mathbf{e}_p + \mathbf{D}_p\left(\dot{p}_d-\dot{\mathbf{p}}_{\mathrm{EE}}\right)
=
\begin{bmatrix}
50   & 0  & 0    \\
0    & 50 & 0    \\
