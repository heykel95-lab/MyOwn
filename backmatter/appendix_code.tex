...
inline DesiredMotion computeDesiredMotion(
  ...
  if (params.constraint_enabled) {
    ...
    // signed_distance = n_s^T (p_plane - p_EE); p_d = p_EE + signed_distance * n_s
...
inline Vec7 computeNullspaceTorque(
  ...
  // Smallest-singular-value direction v_7: last right singular vector of J.
  Eigen::JacobiSVD<Mat6x7> svd_current(J, Eigen::ComputeFullU | Eigen::ComputeFullV);
  Vec7 v7 = svd_current.matrixV().col(6);
  if (v7.norm() <= 1e-9) {
    return limitJointTorqueVectorNorm(tau_nullspace, params.nullspace_tau_max);
  }
  v7.normalize();

  // Compare sigma_min at the two candidate configurations q +/- alpha v_7.
  const Vec7 q_plus = q_current + params.nullspace_alpha * v7;
  const Vec7 q_minus = q_current - params.nullspace_alpha * v7;
