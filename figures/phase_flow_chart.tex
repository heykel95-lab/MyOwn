...
\begin{tikzpicture}[
  ...
  \node[state, text width=2.90cm] (guidance) at (5.00,4.80) {Manual Guidance};
  ...
  \draw[testflow] ($(orient.east)+(0,-0.45)$) -| (9.00,4.80)
      -- (guidance.east);
  \node[testcond, anchor=south] at (8.20,4.95) {\texttt{g}: guide};
  % Recapture enters the top edge vertically instead of ending on a corner.
  \draw[testflow] (guidance.west) -- (2.30,4.80) -- (2.30,1.55)
      -- (1.20,1.55) -- ($(orient.north)+(1.20,0)$);
  \node[testcond, anchor=west] at (2.45,2.60) {\texttt{p}: recapture};
  \draw[flow] (orient) -- (appr);
  % One line: the condition measures 3.73 cm and the rail is 4.07 cm away.
  \node[cond, text width=3.90cm, anchor=west] at (2.15,-1.05)
      {if \(\theta_{\mathrm{app,err}}\leq\varepsilon_{\mathrm{app}}\ \lor\ 5.0\,\mathrm{s}\) timeout};
