...
% end-effector frame so that the pose error used by the impedance law is a
...
\draw[meas] (0,0.10) -- (1.05,0.10) node[lbl, right] {$x_0$};
\draw[meas] (0,0.10) -- (0,1.20) node[lbl, above] {$z_0$};
\draw[gray!60!black] (0.60,0.66) circle (0.11);
\draw[gray!60!black] (0.52,0.74) -- (0.68,0.58);
\draw[gray!60!black] (0.52,0.58) -- (0.68,0.74);
\node[lbl, right, gray!60!black] at (0.74,0.66) {$y_0$};
...
\node[lbl, orange!65!black] at ($(D)+(1.02,0.52)$) {$x_d$};
\draw[des] (D) -- ++(0.34,-0.86);
\node[lbl, anchor=south east, orange!65!black] at ($(D)+(-0.06,0.04)$)
      {$\{d\}$};

% ---- the pose error the impedance law acts on ---------------------------
\draw[err] (EE) -- (D);
...
\begin{scope}[shift={(5.95,-0.10)}, rotate=-7]
  ...
  \node[jt] at (S) {};
  \draw[tsk] (S) -- ++(1.15,0) node[lbl, right] {$t_1$};
  \draw[tsk] (S) -- ++(0,1.05) node[lbl, above] {$n_s$};
  \draw[blue!55!black] (-1.00,0.42) circle (0.11);
  \draw[blue!55!black] (-1.08,0.50) -- (-0.92,0.34);
  \draw[blue!55!black] (-1.08,0.34) -- (-0.92,0.50);
  \node[lbl, left, blue!50!black] at (-1.14,0.42) {$t_2$};
  \node[lbl, below, blue!50!black] at ($(S)+(0,-0.06)$) {$p_s$};
