...
% One panel per commanded tilt direction. Each panel carries the surface
% tangents, the signed tilt axis u_tilt, and the tangential lever selected by
% eq:experimental_general_lever_rule. The lever is the tilt axis turned through
% a right angle, which is what the right-angle marker at the origin records.
%
% The four tilt axes lie at 115, 70, 25 and 160 degrees from t_1, and the
% selected levers therefore lie at 25, -20, -65 and 70 degrees. Those are the
% directions of the lever vectors listed in tab:experimental_case_h_vectors.
%
...
% grey for the frame and the commanded tilt axis.
\begin{tikzpicture}[
    font=\small,
    ax/.style={-{Latex[length=1.8mm]}, gray!60!black},
    tiltax/.style={{Latex[length=1.8mm]}-{Latex[length=1.8mm]}, densely dashed,
                   gray!55!black},
    ...
    10.2/160/{$+45^\circ$ diagonal}}
{
  \begin{scope}[xshift=\dx cm]
    ...
    % ---- commanded tilt axis, drawn through the origin in both senses --
    % The label sits on the far end so it does not crowd the lever label of
    % the neighbouring panel.
    \draw[tiltax] (\ua+180:1.08) -- (\ua:1.08);
    \node[lbl, gray!55!black] at (\ua+180:1.32) {$u_{\mathrm{tilt}}$};
