...
\subsection{Interpretation of the Null-Space Projector}
\label{subsec:interpretation_nullspace_projector}
...
In addition, the singular value decomposition of the active Jacobian is used.
The singular value decomposition is a standard matrix factorization that can be
...
It comes from the general singular value decomposition of a matrix
...
motion in the regulated Cartesian directions.
Large singular values indicate directions in which the active Cartesian task
can be influenced easily.
Small singular values indicate weak directions, where the robot is close to
losing the ability to generate motion in one regulated direction.
...
Here, \(K_{\sigma}\) is a weighting factor for the singular-value
optimization, \(s\) is the selected sign, \(\alpha\) is the joint-space step
size, and \(n\) is the tested null-space direction.
...
\(\tau_g\) and \(\tau_c\) are the model-based gravity and Coriolis
compensation terms.
