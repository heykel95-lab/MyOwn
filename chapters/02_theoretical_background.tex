...
\end{equation}
Thus, gravity is not neglected physically; the robot-side control interface adds
the commanded torque on top of its internal gravity compensation, so the gravity
torque \(\tau_g(q)\) is already taken into account and is not added again in the
user callback. This avoids double compensation and keeps the implementation
consistent with the official libfranka Cartesian impedance example.
