...
is a measure of the kinematic conditioning of the robot.
...
A feedback term toward the start configuration was intentionally not used.
Such a term would have the form
...
q
\right).
\label{eq:start_configuration_feedback}
\end{equation}

In the experiments, this term restricted free motion in the virtual plane,
because it pulled the robot back toward its start configuration.
For this reason, the null-space optimization in this work is used only for
damping redundant motion and for improving the smallest singular value.
