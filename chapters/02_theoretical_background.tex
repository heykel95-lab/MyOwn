...
constructed from the joint angle \(q_i\), the link offset \(d_i\), the link
length \(a_i\), and the link twist \(\alpha_i\).
It is given by
...
Written as a homogeneous transformation matrix, this becomes
...
The remaining parameters \(d_i\), \(a_i\), and \(\alpha_i\) describe the fixed
...
with respect to the previous axis \(z_{i-1}\).
...
A numerical evaluation of the forward kinematics for the Franka Emika Panda at
the home configuration is provided in Appendix A.2, using the parameters listed
in Appendix A.1.
