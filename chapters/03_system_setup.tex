...
Unit (\abbr{FCU}), a workstation PC, and an emergency stop device.
The workstation is connected \textbf{directly} to the FCI Ethernet port
of the \abbr{FCU} with a Cat\,6a cable---no intermediate switch is permitted,
as any additional hop introduces latency sufficient to trigger an \abbr{FCI}
communication timeout.

Static \abbr{IP} addresses are assigned:
...
\midrule
\abbr{FCU} (\abbr{FCI} port) & \texttt{eth0} & \texttt{172.16.0.2} \\
...
\begin{enumerate}
  ...
  \item Activate \abbr{FCI} mode in \textit{Settings} $\to$ \textit{Network}.
  \item Confirm that no error is shown on the status indicator.
  \item Place the emergency stop within reach of the operator.
\end{enumerate}

All experiments are conducted with \abbr{FCI} active and the emergency stop
accessible at all times.

\section{Real-Time Ubuntu Kernel}
\label{sec:rt_kernel}

\abbr{FCI} requires that the control callback completes within 1\,ms.
A standard Linux kernel cannot guarantee this because high-priority threads
can be preempted by interrupt handlers.
The \abbr{PREEMPTRT} patch makes all kernel threads and interrupt handlers fully
...
\begin{enumerate}
  ...
  \item Download and apply the \abbr{PREEMPTRT} patch for the corresponding
        kernel version.
...
$200\,\mu s$ under load before any \abbr{FCI} experiments are conducted.
