...
\mbox{\(53\,\%\)} and \mbox{\(34\,\%\)} below their respective TCP-centred
...
The secondary null-space functions were evaluated individually and together
with a fixed Cartesian pose reference. Projected damping reduced cumulative
joint motion by \mbox{\(25.1\,\%\)} relative to the inactive setting.
At \mbox{\(k_\sigma=2\,\mathrm{N\,m}\)}, adding damping to conditioning
reduced cumulative joint motion by \mbox{\(50.7\,\%\)} relative to
conditioning alone. Both conditioning settings retained the minimum singular
value close to its disturbance-entry value. The joint-angle histories showed
smaller motion with conditioning than without it under the tested disturbance.
...
Projected damping and singular-value conditioning were evaluated individually
and together in Cartesian pose hold under one virtual disturbance direction
and one initial robot configuration. The combined-control trials were acquired
in a later session with matching saved settings and controller revision.
Session effects were not independently controlled. The measured comparisons
therefore remain specific to the investigated condition.
