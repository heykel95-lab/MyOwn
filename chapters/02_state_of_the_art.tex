...
A formal $\SE$ perspective on robot kinematics is given in
...
The Panda's integrated joint torque sensing and \abbr{FCI} torque-control mode
...
The Franka Control Interface (\abbr{FCI}) \cite{FCI2023} exposes a deterministic
1\,kHz torque-control loop via the open-source \texttt{libfranka}
C++ library.
Unlike \abbr{ROS}-based frameworks, libfranka provides direct access to
...
\abbr{FCI} imposes a strict 1\,ms callback deadline; violation triggers
