...
In the actual implementation, the rotational error \(e_R\) is computed from the relative orientation between the desired and current end-effector frames, as described in Chapter~2.
If only translational impedance is used in a specific experiment, the rotational moment can be set to zero.
In that case, the lower three components of \(F\) are set to zero and only the translational impedance force contributes to the task torque.
