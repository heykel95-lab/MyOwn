...
\begin{itemize}
    ...
    \item The resulting Cartesian force/moment vector is mapped to joint torques using \(J^\top(q)\), combined with the null-space torque and model-based compensation terms, and limited before being sent to the robot.
\end{itemize}
...
The controller then computes the Cartesian error, evaluates the impedance law, maps the resulting Cartesian forces and moments to joint torques, adds the projected null-space torque and the model-based compensation terms, and returns the final torque command.
...
Together with the projected null-space torque, they are added after the task torque has been computed, resulting in the commanded torque
...
The gravity compensation term remains the same as in the standard Cartesian
impedance controller. The null-space torque is added as an additional
joint-space contribution before the final torque command is sent to the robot.
...
The singular-value-based part computes a full singular value decomposition of
the active Jacobian. The columns of \(V\) are joint-space directions.
...
The gravity compensation term is obtained from the libfranka model interface
as in the standard controller, and the Coriolis term is added depending on the
selected parameter:
...
Eigen::Matrix<double, 7, 1> tau_raw =
    tau_task + tau_nullspace + gravity;
...
The torque-rate limitation is applied after the task, null-space, model
compensation, and optional Coriolis terms have been combined.
...
robot.control([&](const franka::RobotState& state,
  ...
  Eigen::Matrix<double, 7, 1> tau =
      tau_task + tau_nullspace + tau_g + tau_c;
...
The callback reads the current robot state, evaluates the Cartesian impedance law, adds the active-task null-space torque and the model-based compensation terms, limits the resulting torque command, and returns it to libfranka.
