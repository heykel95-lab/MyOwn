...
No active rotation about a virtual center is used.
The virtual center of rotation is introduced here only as an optional geometric
interpretation of where this passive rotation appears to pivot, and as a
possible extension that would let the pivot point be shaped explicitly. It
couples translation and rotation through a chosen point on, above, or below the
surface.
...
orientation passively, and the virtual center is only the point about which
this measured rotation appears to pivot.
...
for the passive behavior; the virtual-center coupling described in this section
is \emph{not} required for it.

The virtual-center coupling is therefore an optional extension rather than a
missing necessity. It would let the effective pivot point be shaped explicitly,
through a chosen offset \(h\), instead of emerging from the contact geometry. To
add it, the implementation would compute \(p_c=p_s+h n_s\) and \(r_c\) at every
control step, replace the independent position/orientation error by the coupled
error \eqref{eq:implementation_vcr_coupled_error}, and use the corrected
virtual-center wrench \eqref{eq:implementation_vcr_corrected_wrench} in the
torque command. This is a possible refinement of where the passive rotation
pivots, not a prerequisite for self-alignment. Separately, if the surface were
not given in advance, an additional estimation layer would be necessary to
determine \(p_s\) and \(n_s\) from contact forces, external torque estimates,
vision, or another sensor source; that surface-estimation problem is also not
