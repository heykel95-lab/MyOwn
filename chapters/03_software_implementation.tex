...
Experiment-specific files additionally contain the parameter selection and the trajectory or set-point definitions used for each experiment.
...
Listing~\ref{lst:impedance_callback} also shows how the rotational error \(e_R\) is computed from the relative orientation between the desired and current end-effector frames, as described in Chapter~2.
The positional force \(f\) and rotational moment \(m\) are then combined into the Cartesian force/moment vector \(F\) and mapped to joint torques using the Jacobian transpose.
If a specific experiment uses only positional impedance, the rotational moment \(m\) can be set to zero.
In that case, the lower three components of \(F\) are zero and only the positional impedance force contributes to the task torque.
