...
\begin{enumerate}
  \item \textbf{Communication layer}: establishes and maintains the
        1\,kHz FCI connection via \texttt{libfranka}.
...
\begin{tikzpicture}[
  ...
  \node[box, fill=orange!15](fci)   at (0,-1.8) {FCI / \texttt{libfranka}};
...
\begin{itemize}
  ...
  \item \texttt{state.O\_T\_EE}: $\mathbf{T}_e\in SE(3)$
...
\begin{itemize}
  ...
  \item All stiffness matrices are verified to be SPD before controller
