...
\begin{enumerate}
  ...
  \item \textbf{Tracking behavior} --- How accurately and quickly does the
        end-effector converge to the desired pose after a step change?
...
\begin{itemize}
  ...
  \item \textbf{Experiment~4} (\cref{sec:exp_config}): Configuration
        dependence --- same wrench at three different arm poses.
\end{itemize}
...
\begin{equation}
\label{eq:lyapunov}
V(\mathbf{e}_p, \dot{\mathbf{p}}_e) =
\underbrace{\tfrac{1}{2}\,\mathbf{e}_p^\top\,\mathbf{K}_p\,\mathbf{e}_p}_{\text{potential energy}}
+
\underbrace{\tfrac{1}{2}\,\dot{\mathbf{p}}_e^\top\,\mathbf{M}_p\,\dot{\mathbf{p}}_e}_{\text{kinetic energy}}
> 0
\end{equation}

$V > 0$ for all $(\mathbf{e}_p, \dot{\mathbf{p}}_e) \neq (\mathbf{0},\mathbf{0})$
because both $\mathbf{K}_p$ and $\mathbf{M}_p$ are SPD.
This is precisely why SPD is required for both matrices: positive definiteness
...
translational impedance law \cref{eq:imp_trans}:

\begin{align}
\dot{V}
&= \mathbf{e}_p^\top\,\mathbf{K}_p\,\dot{\mathbf{e}}_p
 + \dot{\mathbf{p}}_e^\top\,\mathbf{M}_p\,\ddot{\mathbf{p}}_e
\notag\\
&= -\,\mathbf{e}_p^\top\,\mathbf{K}_p\,\dot{\mathbf{p}}_e
 + \dot{\mathbf{p}}_e^\top\bigl(\mathbf{K}_p\,\mathbf{e}_p
   - \mathbf{D}_p\,\dot{\mathbf{p}}_e\bigr)
\notag\\
\label{eq:Vdot}
&= -\,\dot{\mathbf{p}}_e^\top\,\mathbf{D}_p\,\dot{\mathbf{p}}_e
\;\leq\; 0
\end{align}

where $\dot{\mathbf{e}}_p = -\dot{\mathbf{p}}_e$ (error decreases when
...
$\dot{\mathbf{p}}_e^\top\mathbf{D}_p\dot{\mathbf{p}}_e \geq 0$.
Strict inequality ($\dot{V} < 0$ for $\dot{\mathbf{p}}_e \neq \mathbf{0}$)
...
the equilibrium $(\mathbf{e}_p, \dot{\mathbf{p}}_e) = (\mathbf{0},\mathbf{0})$ is
\textbf{stable}.
...
permanently (i.e.\ $\dot{\mathbf{p}}_e \equiv \mathbf{0}$) is the
equilibrium itself.
The same argument applies to the rotational subsystem with $\mathbf{e}_R$,
$\mathbf{K}_R$, $\boldsymbol{\omega}_e$, $\mathbf{D}_R$.
...
\begin{enumerate}
  \item The robot is held at the desired pose $\mathbf{p}_d$.
  ...
  \item The system must return to equilibrium without oscillation
        (critical damping defined by \cref{eq:D_base_local}).
...
designed according to \cref{subsec:KD_matrices}.


For later comparison, the critically damped single-axis response is described by

\begin{equation}
\label{eq:settling}
t_s \approx \frac{4}{\omega_n}=\frac{4}{\sqrt{K_i}},
\qquad \omega_n=\sqrt{K_i},
\end{equation}

where \(t_s\) is the approximate 2\,\% settling time and \(K_i\) is the
stiffness in the investigated principal direction for unit virtual mass. Under
...
\caption{Stability verification: Lyapunov function $V(t)$ (left) and its
         time derivative $\dot{V}(t)=-\mathbf{D}_p\dot{p}_e^2\leq0$ (right)
...
base frame using \cref{eq:K_base_local,eq:D_base_local}. This experiment
therefore demonstrates that full matrices are meaningful even when the task is
only to track one fixed point.

The complete C++ implementation, simulation code, and parameter table are
given in Appendix~\ref{app:exp1}.
...
\begin{enumerate}
  ...
  \item Activate the impedance controller and record
        \(\mathbf{p}_e(t)\), \(\dot{\mathbf{p}}_e(t)\),
...
\begin{itemize}
  \item position error components \(e_x(t),e_y(t),e_z(t)\);
...
\begin{enumerate}
  \item The robot is moved to the initial pose on the wall surface
...
\begin{itemize}
  \item \textbf{Normal displacement $d(t)$} from \cref{eq:wall_dist}:
        should remain near zero during in-plane trials and return to zero
        after a normal push.
  \item \textbf{In-plane position} $\mathbf{p}_e \cdot \hat{\mathbf{t}}_1$
        and $\mathbf{p}_e \cdot \hat{\mathbf{t}}_2$ vs.\ time:
        should show free motion during in-plane trials.
  \item \textbf{Tilt angles} about $\hat{\mathbf{t}}_1$ and $\hat{\mathbf{t}}_2$
        extracted from $\mathbf{R}_e$ vs.\ time: should converge freely to
...
The complete simulation code and parameter table are in
...
end-effector pose and the same stiffness parameters.
...
joints depending on arm pose.
The complete simulation code is in Appendix~\ref{app:exp4}.
...
Under-damping ($D_i < 2\sqrt{K_i}$) causes oscillatory transients;
over-damping ($D_i > 2\sqrt{K_i}$) increases settling time without
improving steady-state accuracy.
The optimal critical-damping condition $D_i = 2\sqrt{K_i}$ (with
$M_i=1$) is computed analytically.
In practice, a $\pm20\%$ deviation from the critical value is
acceptable: under-damping by 20\% produces a peak overshoot of
approximately 1.5\%, which is within the positioning tolerance of
the FCI sensor.

\subsection{Sensitivity to Model Errors}

The gravity and Coriolis compensation terms $\bm{\tau}_g$ and
$\bm{\tau}_c$ are computed from the libfranka model.
An error in these terms appears as a constant force offset in the
Cartesian space:
...
\begin{itemize}
  \item \textit{``The position error $\|\mathbf{e}_p(t)\|$ converged
        ...
        the critical-damping condition $\mathbf{D}_p = 2\mathbf{K}_p^{1/2}$
...
\begin{itemize}
  \item \textit{``The measured settling time at $K_x=200\,\mathrm{N/m}$
        ...
        prediction $4/\omega_n = 4/\sqrt{200} \approx 0.28\,\mathrm{s}$
        from \cref{eq:settling}.''}
...
\begin{itemize}
  ...
  \item \textit{``The torque distribution changed significantly across the
        three configurations even for the same Cartesian wrench, demonstrating
