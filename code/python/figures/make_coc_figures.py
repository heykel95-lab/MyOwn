...
# labels name the achieved entry condition.
GROUPS = [
    ("P2_t1_pos", "t1"),
    ("P2_t1_neg", "t1"),
    ("P2_t2_pos", "t2"),
...
def initial_deviation_label(groups, runs, axis, linebreak=False):
    """Name a series by its mean achieved angular offset."""
    separator = "\n" if linebreak else " "
    ...
    if not values:
        return f"Achieved Angular Offset About ${tangent}$"
    value = -statistics.mean(values)
    return (f"Achieved Angular Offset,{separator}"
            f"$\\theta_{{\\mathrm{{ach}},{tangent}}}={value:+.2f}^\\circ$")
...
def stiffness_label(case, axis, groups, runs):
    ...
    if values:
        ...
        return (f"About ${tangent}$, ${entry}$\n"
                f"$\\theta_{{\\mathrm{{ach}},{tangent}}}={angle}^\\circ$")
    return f"About ${tangent}$, ${entry}$"
...
def main():
    ...
    ax.set_xlabel(r"Achieved Angular Offset, $\theta_{\mathrm{ach},t_1}$ [$^\circ$]")
