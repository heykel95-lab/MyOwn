...
be read against the load that produced it.
  ...
  rotation   the turn since contact-establishment entry, resolved along the surface axes. It
...
def load(trial):
    ...
    with open(logs[0]) as f:
        for row in csv.DictReader(f):
            if float(row["phase"]) != CONTACT_ESTABLISHMENT_STATE:
                continue
...
def main():
    ...
    for column, (trial, label) in enumerate(selected):
        ...
        axes[2][column].set_xlabel(f"Time from contact-establishment entry [s]\n{label}")
