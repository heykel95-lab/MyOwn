...
Current null-space figures (2026-09-21): the three main-presentation plots
are now included by Chapter 5. See `nullspace_presentation_main/README.md`
for portable sources and verification. The earlier null-space plot entries
below are historical and remain archived. Other result figures are unchanged.
...
below still runs and every printed number is identical, but the tick locator
chooses fewer ticks. A panel of `MAIN_D_wrench.pdf`, for example, carries ticks
every 5 rather than every 2.5.
...
`make_nullspace_figure.py` integrates the magnitude of the projected joint
velocity to obtain cumulative projected joint motion across all seven joints.
It also evaluates the minimum singular value and Cartesian position error over
the exact recorded disturbance interval. The active thesis figures add the
measured joint-1 history as the temporal example.

Running the script against `MyController/experiments/results/` reproduces the
cumulative-motion and conditioning quantities used by the thesis.
...
Regenerate the PDF in the same turn as a label edit. A source-only label change
is invisible until someone reads the compiled figure against the paragraph
beside it.
